...
RASPBIAN_IMG_VERSION="2016-09-23"
...
echo -e "${OK_COLOR}== Raspbian ${RASPBIAN_VERSION} for Raspberry Pi ==${NO_COLOR}"
if [ $# -eq 0 ]; then
    display_usage
fi
...
if [ ! -f "${RASPBIAN_IMG_VERSION}-raspbian-jessie-lite.zip" ]; then
    curl -LO --progress-bar http://director.downloads.raspberrypi.org/raspbian_lite/images/raspbian_lite-${RASPBIAN_VERSION}/${RASPBIAN_IMG_VERSION}-raspbian-jessie-lite.zip
fi

echo -e "${WARN_COLOR}Extracting the image${NO_COLOR}"
if [ ! -f "${RASPBIAN_IMG_VERSION}-raspbian-jessie-lite.img" ]; then
     unzip ${RASPBIAN_IMG_VERSION}-raspbian-jessie-lite.zip
fi

setup_sdcard $sdcard $os

echo -e "${WARN_COLOR}Installing OSMC to SD Card${NO_COLOR}"
flash_sdcard ${sdcard} ${RASPBIAN_IMG_VERSION}-raspbian-jessie-lite.img ${os}
